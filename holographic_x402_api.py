...
<section class="strip" aria-label="Live deployment details"><div><strong>Endpoint</strong><span>$public_url</span></div><div><strong>Payment</strong><span>x402 exact scheme</span></div><div><strong>Buyer shape</strong><span>inspect, pay, call</span></div></section>
...
def create_app(
    ...
    async def remember(payload: Dict[str, Any], x_admin_token: Optional[str] = Header(default=None)) -> Dict[str, Any]:
        ...
        # Without this, admin writes live only in-process and vanish on restart.
        if state_path:
