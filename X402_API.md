...
but they cannot mutate the shared memory store unless they also hold the admin
token.
...
export LECORE_X402_PRICE="$0.001"
...
- Keep route prices explicit; avoid wildcard paid route configs for this first
  product surface.
- Keep writes admin-only, or move customer writes into isolated per-customer
  stores before charging for them.
...
For hosting, see [`FLY_X402_DEPLOY.md`](FLY_X402_DEPLOY.md) (default) or
[`AWS_X402_DEPLOY.md`](AWS_X402_DEPLOY.md) (alternative; also holds the full
wallet-storage discussion).
