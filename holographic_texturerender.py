...
def _object_uv(sdf, P):
    ...
    from holographic_semantic import _SphereSDF, _BoxSDF
    local = P - sdf.c
...
def _is_texture_graph(tex):
    ...
    from holographic_texturegraph import Node
...
def _albedo_rough_metal(tex, uvs, base_color, mat_name):
    ...
    if _is_texture_graph(tex):
        ...
        from holographic_preview import _channel_names, _sample_scalar
...
def render_textured(scene, textures, camera=None, width=256, height=192, light_dir=(0.5, 0.85, 0.6),
    ...
    from holographic_semantic import _UnionSDF, COLORS
