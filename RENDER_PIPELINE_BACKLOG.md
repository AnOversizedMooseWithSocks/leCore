...
**DONE (batch 3):** H7 -- the renderer now consumes the canonical SCENE DOCUMENT. New holographic_scene_render.py flattens a holographic_scene_doc.Scene into (sdf, material_fn): every object placed by its transform, union-ed into one scene SDF, each hit shaded by its owning object's LIBRARY material; render_scene_document renders it in one call. Wired as UnifiedMind faculties (render_scene_document / scene_to_render) + integration test + tour block. **DONE (batch 16):** BUILD CACHED INDIRECT / GI (RENDER-DC3, holographic_lightcache.cached_indirect_shade) -- the real placed-light speckle fix. The dominant speckle is GI bounce noise (~73%, measured), so cache the one-bounce indirect the same three-tier way: bake a many-ray hemisphere gather (real scene lights+materials) at coarse anchors, interpolate, recompute edges; tracer renders DIRECT-only and adds the clean cached GI. PRODUCTION result: bake caches once (2.8s) then render -> placed-light speckle 0.0337->0.0044 = 87% LESS and ~8x faster; real colour bleeding. Wired render_scene_document(indirect_cache=False)+mind. KEPT NEGATIVE: one bounce not full GI (~5% dimmer); diffuse only. Completes the two-mode cached lighting: DC1 dome + DC2 soft area lights + DC3 indirect, one shared engine.

**DONE (batch 15):** BUILD CACHED SOFT AREA LIGHTS (RENDER-DC2, holographic_lightcache.py) + fixed a ROOT-CAUSE bug: area lights (Rect/Disk/Sphere/Mesh) lacked the .soft flag, so they were sampled with 1 shadow ray (biased hard shadow = the penumbra speckle). Flagged them soft (now multi-sampled) and cached the soft term via a generalised three-tier engine (cached_screen_shade, now shared by the dome). Cached soft light is noise-free (seed-diff 0.0045, ~8x cleaner) at 96% hit rate. Wired render_scene_document(soft_light_cache=False)+mind, default-off. HONEST DIAGNOSIS: cleans the DIRECT soft term but only ~6% of the showcase speckle -- measured ~73% is INDIRECT/GI bounce noise (seed-diff 0.010 direct-only vs 0.038 with bounces). NEXT: cache the indirect bounce irradiance the same way.

**DONE (batch 14+):** BUILD M5 (super-res demodulation, holographic_modulate.superres_demodulated + render_demodulated_upscale) -- render lighting low-res, upscale the smooth irradiance, remodulate the crisp high-res albedo. 2.6x faster than full high-res AND 18% cleaner than plain upscale on textured content; carrier from the anti-aliased downsampled high albedo (the fix that flipped it from -19% to +18%). Wired UnifiedMind.render_demodulated_upscale. KEPT NEGATIVE: texture-only, diffuse-only, background masked.

**DONE (batch 14):** BUILD M1 (modulate/demodulate primitive = bind/unbind, holographic_modulate.py) + M4 (demodulated denoise: divide albedo out, denoise smooth irradiance, multiply back). Probed first -- the 5 bakes (matcompile/matbake/viewlut/prt/radiance) already exist, svgf was guide-only, no demodulate primitive. Measured 33% less denoise error on textured diffuse; KEPT NEGATIVE: neutral on uniform albedo (so NOT the matte-speckle fix), diffuse only, background masked. Wired render_auto(demodulate=False)->render_scene_document->mind, default-off/byte-identical. NEXT for the matte speckle specifically: extend the dome-style soft-light cache to the area lights.
