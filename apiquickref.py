...
def generate(root="."):
    ...
    out = ["# leCore API Quick Reference",
           "",
           # NO DATE STAMP. ci.yml GATES this file: it regenerates and `git diff --exit-code`s the committed
           # copy. A date.today() stamp made the output a function of the CALENDAR, so the gate went red on any
           # push made on a later day than the last docs commit -- zero code changed. Measured: the only diff
           # between the committed and regenerated copy on a red run was `on 2026-07-09` -> `on 2026-07-16`.
           # capdoc.py (same gate, no timestamp) has never false-failed -- that is the control. A generated
           # file that is gated must be a PURE FUNCTION OF THE SOURCE, or the gate measures the clock.
           "*A scannable, one-line-per-symbol map of the app-building surface -- auto-generated by "
           "`apiquickref.py`. For the full engine (every module), see REFERENCE.md.*",
           ""]
