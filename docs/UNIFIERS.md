...
*ONE weighted Dijkstra over the mesh edge-graph, returning (dist, prev). `geodesic_distances` kept its distance field; `meshseam.shortest_seam` had reimplemented the SAME adjacency build + the SAME Dijkstra inline (a distance FIELD vs a PATH -- one algorithm in two costumes) and did not even import meshgeodesic. Lifted to (dist, prev) with an optional early-out `target`; shortest_seam now delegates and only reconstructs the path from prev. TIE-SENSITIVE path (both documented vertex-index tie-breaking): verified BYTE-IDENTICAL on 18 golden outputs (distance hashes + exact seam paths) before/after -- a behaviourally-invisible dedup, pinned by a regression trap.*
| client | status |
| `holographic_meshseam` | ✅ wired |

## `honesty.permutation_null`  — primitive shipped; 5 private-null clients DEFERRED (measured)
*The shuffled-null discipline (score a datum, re-run the identical scoring on structure-destroying resamples, demand the real score stand out -- Tarter's radio-SETI + Cranmer's particle-physics move) as ONE composable primitive, callable by any capability including one built on the engine. Generalises the five procedure-matched private nulls (`_recall_null`, `_recognition_null`, `_brain_null`, `_scan_cue_null`, and the MI/phase-randomised nulls). VERIFIED bit-identical to the hand-rolled recall-null loop those five share; adds the +1 plug (North et al. 2002) so p is never exactly 0. Calibrated (random datum flags at ~alpha) and deterministic. Wired as `mind.permutation_null`.*
| client | status |
| the 5 private nulls | ⏸ DEFERRED (measured) — the primitive subsumes them bit-identically, but they return a RecallNull object (not the summary dict), are cached on domain-specific mutation counters, and feed pinned decide_confidence/recognize/scan thresholds; the +1 plug shifts p by ≤1/(n+1), correct but would move those pins. Per ledger discipline the primitive ships first; delegation + re-pinning is the measured follow-up, NOT forced. |

## `spatial.SpatialGrid (one shared knn index)` — registered (scope: LOW-D geometric index); the 3 proposed clients retracted as NOT_APPLICABLE (measured, G2)
*The proposal to route spectral/graphsignal/chart k-NN onto SpatialGrid was MEASURED and REJECTED. A uniform grid is a LOW-D geometric index; these callers operate in HIGH dimensions. A single high-D `grid.knn` query did not finish in 60 s (ring search explodes over near-empty cells above ~3 D), and even in its own low-D regime the grid LOST to the vectorised dense scan until N was very large (0.16× at N=300 D=2; ~tie 0.96× at N=4000 D=3). The genuinely shared high-D index is the HoloForest — which chart+graphsignal ALREADY use via a `forest=` hook. G2's real, measurement-justified fix: gave `spectral.knn_adjacency` the SAME `forest=` hook (measured 1.31× at N=800 D=128, growing with N; byte-identical when `forest=None`). Different index for a different regime is not a unification target.*
