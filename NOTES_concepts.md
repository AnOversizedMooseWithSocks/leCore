...
## Sweep 176, CI fixes (20 Sep 2026) -- three gates the sweep tripped, each fixed at its cause

  * tests/test_unified_split.py: holographic_unified_p08_bake.py reached 2,383 lines (cap 2,000). The
    sweep's decision faculties now live in their own part, holographic_unified_p27_decisions.py
    (_UnifiedPart27, registered in the shim): decision_tree / decision_memory / decision_ledger /
    decision_outcome / decision_records / decision_ledger_to_memory / reflex_learn / reflex_decide /
    reflex_retile / verify_decision / systemone_batch_fdr / systemone_absorb / swarm_step / swarm_evaluate
    / plan_from_request and the private helpers -- moved by line range, byte-identical; check_part reports
    17 members reached UnifiedMind, none shadowed. p08 is back to 1,914 lines. RULE FOR NEXT TIME: new
    decision/reflex verbs go in p27, not p08.
  * tests/test_duplication_audit.py: three new public names collide with unrelated modules -- grow_tree
    (tree3d grows a 3-D skeleton by space colonisation; decisiontree grows a contingency tree), encode_tree
    (typed's kernel expression-tree encoding; decisiontree's schema-carrying wrapper over encode_plan),
    read_tree (codestructure reads a directory of text files; decisiontree reads a tree vector back to a
    PlanNode). Both bodies of each read; nothing to unify; budgeted in tools/name_collisions.KNOWN_COLLISIONS
    with the reasons on the lines.
  * tests/test_doc_coverage.py: unmentioned mind verbs grew 1984 -> 1998. FEATURE_GUIDE.md section 12
    ("Typed decisions that learn") documents every door the sweep added, with the measured numbers;
    unmentioned is now 1,973, inside the budget. No --rebase.
Verification: the three gate files pass; audits 0/0/0/0; docgen 817 modules; 7,284 tests collected.

## Sweep 176, docs pass (20 Sep 2026) -- the Jev/NOOA surface documented, with examples that cannot drift

  * docs/TYPED_DECISIONS.md: the syntax reference -- schema (choice / noul / score; anchors are (text, value)
    pairs, a value-keyed dict is refused -- caught by generating the examples live), the answer shape, outcomes
    by id, route_tiered's tiers, the seen gate, verify_decision's verdict, swarm_step's validated termination,
    the NOOA table, the HTTP form. Every JSON block is GENERATED by tools/gen_typed_examples.py from the live
    engine; regenerate after any change to these doors (PYTHONPATH=. -- a PyPI leos-core shadowed the checkout
    once and the mind "had no attribute systemone_lint").
  * AGENTS.md: "DECIDE, DON'T PROMPT" block with the six calls; doc links; counts. README: the decision surface
    in "What can it do?". docs/WHATS_NEW.md sweep-176 entry; docs/INDEX.md and docs/DOC_MAP.md linked.
  * skills/lecore-boot/SKILL.md (Step 7b: the typed doors over /invoke, schema syntax, the model end,
    reflex_retile after boot, the sandbox lessons), skills/lestudio/SKILL.md (the benchmark-poster swarm
    recipe; verify text by geometry), skills/lecore-render/SKILL.md (kill-safe strips, the __main__ guard,
    sdf_scene_shader, the exact-SDF path). Copies for the skill folders; the repo carries them under skills/.
  Verification: doc_coverage within budget (1,973 / 1,984); the three CI gates + the sweep's tests pass.

## Sweep 176, the code workflow (20 Sep 2026) -- plan, edit under validated termination, review with evidence

Moose: "make sure code creation, review, management, editing and planning are handled as best they can;
use the Jev and NOOA capabilities and the planning tools." Rule 0 found every PIECE present (code_similar
is Rule 0 asked of the source; function_purity; affected_tests; the file_* checks; file_undo; the audits)
and the COMPOSITION absent: "review a change", "reuse / extend / build", "edit and verify", "check
determinism" all returned fallbacks. holographic_codeflow.py (selftest 9 pins) + three verbs in part 27:
  * plan_change(request): evidence = route_tiered tier/z + code_search + family, written into the typed
    decision's state; the measured tier decides where decisive (answer -> reuse, refuse -> build),
    MODIFICATION INTENT ("add a parameter to X") turns an answer-tier hit into extend, the typed decision
    breaks the menu tie and learns from decision_outcome. Steps = the build loop with a done_when each.
    MEASURED on 12 historical requests: 6/12 against the historical truth, 9/12 against TODAY's truth
    (three "misses" were hindsight -- the things exist now, so reuse is right), 11/12 after the intent rule.
    Two of the original misses were CATALOG GAPS the plan exposed (the audits and drift_report had no
    reachable aliases) -- fixed. KEPT: four seed examples per option outvoted an answer-tier hit ("smooth a
    bumpy mesh" -> build) until the rule took the decisive tiers; a scorer with no history loses to a
    measured signal.
  * edit_verified(path, old, new, import_check, selftest_module): NOOA validated termination on one edit --
    any failed check undoes it (file_undo) and refuses with the check attached; an accepted edit is a swarm
    step. MEASURED, 200 synthetic edits: 100/100 broken refused + restored byte-identical, 100/100 good
    accepted. Used live to fix the reviewer's own finding in systemone (predict() had no docstring) with
    the selftest as the gate.
  * review(paths): syntax, import (subprocess -- the checker takes the PATH; a dotted name was the first
    bug), determinism hazards from the AST (hash / unseeded random / wall clock / unsorted fs / set
    iteration; comments and docstrings cannot trip it), undocumented public defs (module-level and class
    methods only -- a nested helper was flagged first), long functions, the 2,000-line part cap, no
    selftest, duplicates by code_similar, impure functions, affected tests. merge_ready = no errors, never a
    score. Its own module reviews clean; systemone had one real gap, now fixed.
