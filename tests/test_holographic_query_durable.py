...
import tempfile
import pytest
from holographic.agents_and_reasoning.holographic_query import Database, run_sql, update as _update
from holographic.agents_and_reasoning.holographic_query_durable import save_snapshot, load_snapshot, Journal, recover
...
def test_journal_delete_recovered():
    d = tempfile.mkdtemp(); snap = os.path.join(d, "s.json"); jrnl = os.path.join(d, "j")
    db = _db(); save_snapshot(db, snap)
    from holographic.agents_and_reasoning.holographic_query import delete as _del
