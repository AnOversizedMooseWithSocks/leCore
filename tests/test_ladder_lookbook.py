...
def test_the_z_floor_is_not_a_significance_test(mind):
    ...
    # A FLOOR IS A FLOOR: the quantised values sit at ~0.40/0.54 (no optional plugins) or ~0.47/0.61 (all of
    # them), so the search also steps the floor down until some candidate barely clears it -- the relationship
    # (cleared a floor, p far from significant, p decoupled from the strong query's) holds at any floor.
    weak = None
    for z_min in (0.5, 0.3, 0.1):
        for q in ("transform", "denoise", "interpolate", "encode", "quantize", "compress", "cluster", "rotate", "tile", "bind"):
            r = mind.route_or_abstain(q, z_min=z_min)
            if r["abstain"] is False and r["z"] < 2.0:
                weak = r
                break
        if weak is not None:
            break
    assert weak is not None, "no candidate barely clears any floor down to 0.1 in this catalog; extend the list"
    assert weak["p"] > 0.05, (
