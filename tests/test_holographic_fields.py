...
from holographic.misc.holographic_fields import diffuse, divergence, curl, project_divergence_free, advect, fluid_step, ParticleSystem, attractor_force, sample_field
...
def test_vorticity_confinement_preserves_curl():
    from holographic.misc.holographic_fields import smoke_step, curl
    def run(conf):
        vx = np.zeros((H, W)); vy = np.zeros((H, W)); dens = _blob(16, 6); temp = _blob(16, 6)
        for _ in range(30):
...
def test_enforce_solid_zeros_velocity_in_the_mask():
    from holographic.misc.holographic_fields import enforce_solid, disc_mask
    rng = np.random.default_rng(0)
    vx = rng.normal(size=(H, W)); vy = rng.normal(size=(H, W))
...
def test_obstacle_blocks_and_diverts_flow():
    from holographic.misc.holographic_fields import fluid_step, disc_mask
    solid = disc_mask((H, W), center=(16, 16), radius=5)
...
def test_smoke_flows_around_an_obstacle():
    from holographic.misc.holographic_fields import smoke_step, disc_mask
...
def test_diffuse_3d_conserves_mass_and_smooths():
    from holographic.misc.holographic_fields import diffuse_3d
    f = _blob3(8, 8, 8); d = diffuse_3d(f, 2.0)
    assert abs(f.mean() - d.mean()) < 1e-9 and d.var() < f.var()


def test_projection_3d_removes_divergence():
    from holographic.misc.holographic_fields import divergence_3d, project_divergence_free_3d
    rng = np.random.default_rng(0)
...
def test_advect_3d_moves_blob_by_v_dt():
    from holographic.misc.holographic_fields import advect_3d
    dens = _blob3(4, 8, 8)
...
def test_fluid_step_3d_stays_incompressible():
    from holographic.misc.holographic_fields import fluid_step_3d, divergence_3d
...
def test_spectral_field_tiles_seamlessly():
    from holographic.misc.holographic_fields import spectral_field, seam_continuity
...
def test_sphere_mask_obstacle_blocks_3d_flow():
    ...
    import numpy as np
    from holographic.misc.holographic_fields import sphere_mask, fluid_step_3d
    N = 20
...
def test_sample_scatter_3d_are_adjoint():
    ...
    import numpy as np
    from holographic.misc.holographic_fields import sample_field_3d, scatter_to_field_3d
...
def test_pairwise_repulsion_disperses_a_clump():
    ...
    import numpy as np
    from holographic.misc.holographic_fields import pairwise_repulsion, ParticleSystem
    rng = np.random.default_rng(1)
