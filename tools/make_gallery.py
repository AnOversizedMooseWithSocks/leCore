...
def _save_render(name, scene, cam, material, max_bounce=4, bench=False, sky=None, dispersion=0.0, caustics=None):
    ...
    from holographic.rendering.holographic_gbuffer import render_auto, render_dispersion, add_caustics
...
def render_light_types():
    ...
    from holographic.rendering.holographic_lights import SpotLight, RectLight, IESLight, DomeLight
...
def render_lit_scene():
    ...
    from holographic.mesh_and_geometry.holographic_sdf import sphere, box
    from holographic.rendering.holographic_render import Camera
    from holographic.rendering.holographic_lights import SphereLight, PointLight
...
def render_hot_metal():
    ...
    from holographic.mesh_and_geometry.holographic_sdf import box, cylinder, sphere
    from holographic.rendering.holographic_render import Camera
    import holographic.materials_and_texture.holographic_matlib as ML
...
def render_cloud():
    """CLOUD -- the multi-lobe fBm cumulus (holographic_semantic.cloud_field) through the physically-lit volume
    renderer: single-scatter self-shadowing (bright crown, sky-blue shadowed base), a Henyey-Greenstein forward
    lobe for the silver lining, the Beer-Powder term for roundness, and a cheap multi-scatter approximation so the
    shadows are not black. The noise density is baked with the exact-but-vectorised sample_grid_fast, so this stays
    quick. See RENDERING_GUIDE.md and holographic_render.volume_render(self_shadow=...)."""
    from holographic.simulation_and_physics.holographic_semantic import cloud_field
    from holographic.rendering.holographic_render import volume_render, Camera, Light
    field = cloud_field((0, 0, 0), 1.3, density=3.4, seed=11, grid=40)
    cam = Camera(eye=(2.5, 0.18, 3.4), target=(0, 0.05, 0), fov_deg=38, aspect=WIDTH / HEIGHT)
    bounds = (np.array([-2.0, -1.1, -2.0]), np.array([2.0, 1.4, 2.0]))
    sun = Light("directional", direction=(-0.42, -0.5, -0.62))
    img, alpha = volume_render(field, cam, bounds, WIDTH, HEIGHT, steps=150, mode="smoke", sigma=6.5,
                               albedo=(1.0, 0.99, 0.97), lights=[sun], self_shadow=True, shadow_steps=24,
                               shadow_sigma=9.0, ambient=(0.40, 0.52, 0.74), phase_g=0.78, powder=True,
                               multi_scatter=4)
    yy = np.linspace(0, 1, HEIGHT)[:, None, None]
    sky = np.array([0.18, 0.38, 0.72]) * (1 - yy) + np.array([0.66, 0.80, 0.95]) * yy
    a = alpha[..., None]
    final = np.clip(img * a + sky * (1 - a), 0, 1)
    plt.imsave(f"{OUT}/render_cloud.png", final)     # already display-ready [0,1]; no tonemap needed
    print("  render_cloud.png")
...
def render_ocean():
    ...
    from holographic.rendering.holographic_raymarch import sky_dome
    from holographic.rendering.holographic_globalillum import caustics as _caustics
    W, H = WIDTH, HEIGHT
...
def render_patterns():
    ...
    import holographic.misc.holographic_pattern as P
...
def chart_compression():
    ...
    from holographic.misc.holographic_ratedistortion import geometry_preserving_code, pack_code
...
def _kv_recall(D, K, corrupt=0.0, seed=0):
    ...
    from holographic.agents_and_reasoning.holographic_ai import bind, unbind, bundle
