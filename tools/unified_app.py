...
def load_self():
    ...
    import io, tokenize, importlib.util
    mods = [("holographic.io_and_interop.holographic_image", "code:image"),
            ("holographic.misc.holographic_creature", "code:creature"),
            ("holographic.misc.holographic_tree", "code:tree"),
            ("holographic.misc.holographic_text", "code:text"),
            ("holographic.agents_and_reasoning.holographic_ai", "code:engine")]
    items, raw = [], []
    for modname, lab in mods:
        # resolve the module's source file wherever it lives (clone or installed wheel), via its import spec
...
def load_onchain_world():
    ...
    from holographic.misc.holographic_market import load_onchain_traders
...
def codec_endpoint():
    ...
    from holographic.misc.holographic_text import _tokens
...
def market_endpoint():
    ...
    import numpy as _np
    from holographic.misc.holographic_signal_structure import volatility_clustering, clustering_zscore
...
def discover_endpoint():
    ...
    from holographic.misc.holographic_segment import Segmenter, boundary_f1, chunk_compression
    seg = Segmenter(dim=512, order=4, seed=0).fit(stream)
    bounds = seg.boundaries(stream)
...
def compose_endpoint():
    ...
    from holographic.misc.holographic_compose import render_scene, render_fidelity
    j = request.json or {}
...
def nested_endpoint():
    ...
    faculty (one brain)."""
    import holographic.scene_and_pipeline.holographic_scene as hs
    from holographic.misc.holographic_compose import render_scene
    j = request.json or {}
    mind = STATE.get("mind") or UnifiedMind(dim=1024, seed=0)
    rng = np.random.default_rng(int(j.get("seed", 0)))
...
def highcap_endpoint():
    ...
    import numpy as _np
    from holographic.agents_and_reasoning.holographic_ai import random_vector, bind, unbind, cosine
    j = request.json or {}
...
def morph_endpoint():
    ...
    import holographic.scene_and_pipeline.holographic_scene as hs
    from holographic.misc.holographic_generate import crossfade_images, ghosting
    j = request.json or {}
    mind = STATE.get("mind") or UnifiedMind(dim=1024, seed=0)
    rng = np.random.default_rng(int(j.get("seed", 0)))
...
def nucleus_endpoint():
    ...
    if _need_mind() or STATE["mind"]._gen is None:
        return jsonify({"error": "load a dataset first"})
    from holographic.misc.holographic_generate import generate_text, real_word_fraction, distinct_ngram_fraction
    mind = STATE["mind"]
