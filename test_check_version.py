...
def _mod():
    path = os.path.join(os.path.dirname(__file__), "tools", "check_version.py")
...
def test_reads_the_version_from_setup():
    cv = _mod()
    v = cv.read_version(os.path.join(os.path.dirname(__file__), "setup.py"))
    assert v and all(part.isdigit() for part in v.split("."))     # looks like N.N.N


def _run(*args):
    tool = os.path.join(os.path.dirname(__file__), "tools", "check_version.py")
