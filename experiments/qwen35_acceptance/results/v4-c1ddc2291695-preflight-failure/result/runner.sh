...
RUN_ROOT=/opt/qwen35-acceptance-v4
...
PREFIX=qwen35-acceptance/c1ddc22-2fc06364-a6c9d113/v4-formal
LECORE_COMMIT=c1ddc22916957238c00890bec13fd0609b01b521
...
LAUNCH_SHA=052794ed379e799aa13c7eee9a649570b5fa80017134aa44756d01a30200d30b
STAGE=boot
...
# this fires. Eight hours of r7i.4xlarge compute is $8.4672, preserving the
# authorized $10 total ceiling after storage and transfer.
shutdown -h +480
...
with open(path, "w", encoding="utf-8") as handle:
    json.dump({
        "schema": "lecore.qwen35-runner-status.v4",
        "stage": stage,
        "exit_code": int(exit_code),
        "formal_run_started": formal == "true",
        "experiment_id": experiment_id or None,
        "source_commit": "c1ddc22916957238c00890bec13fd0609b01b521",
...
"${PYTHON}" "${LECORE_DIR}/experiments/qwen35_acceptance/generate.py" \
  ...
  --experiment-version 4 --initial-chunk-size 128 \
...
checks = {
    ...
    "experiment_version": project["experiment_version"] == 4 and project["experiment_id"].endswith(".v4.acceptance"),
    "policy_digest": project["runner_policy_digest"] == policy_digest == experiment["runner_policy_digest"],
    "policy_identity": policy_digest[:12] in project["experiment_id"],
    "checker_version": experiment["evidence_authority"]["provenance"]["checker"] == "checker://lecore/qwen35-acceptance/%s/v4" % policy_digest,
    ...
    "progress_bound": policy["telemetry"]["progress_upload_uri"] == "s3://zero-training-022118847419/qwen35-acceptance/c1ddc22-2fc06364-a6c9d113/v4-formal/progress.jsonl",
    "launch_source": launch["source"]["commit"] == source_commit,
    "launch_attempt": launch["attempt"]["experiment_version"] == 4 and launch["attempt"]["maximum_formal_runs"] == 1 and launch["attempt"]["retry_after_admission"] is False,
    "launch_ceiling": launch["aws"]["total_ceiling_usd"] == 10.0 and launch["aws"]["maximum_compute_usd"] <= 8.4672,
}
...
payload = {
    "schema": "lecore.qwen35-execution-summary.v4",
