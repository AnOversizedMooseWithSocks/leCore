...
def _pipe_gbuffer(r):
    ...
    from holographic.caching_and_storage.holographic_domecache import _primary_gbuffer
...
def _strat_radiance(ctx):
    ...
    from holographic.rendering.holographic_radiance import reconstruct_view
    r = ctx.scene
    hit, P, N = _pipe_gbuffer(r)
...
def _present_run(ctx):
    ...
    if img is not None:
        ...
        if cfg is not None and getattr(cfg, "postfx", "off") == "aces":
            from holographic.rendering.holographic_postfx import auto_exposure, aces, gamma
