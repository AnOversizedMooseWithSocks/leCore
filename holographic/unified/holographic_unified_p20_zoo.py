...
class _UnifiedPart20:
    ...
    def explain(self, topic, other=None):
        """DOCS-DERIVED EXPLANATION, PUSHED DOWN (cp68): 'how does X work' answered
        from the generated documentation. POLYMORPHIC (sweep 114): with a second
        argument this is the RELATIONS explain -- explain(a, b) -> per-role
        [(role, v1, v2, shared, confidence), ...] via explain_similarity, the body
        sweep 63 renamed out from under app.py / unified_app.py / the relations
        tests, which kept calling the two-argument form and went red for the life
        of the rename. One-argument callers are byte-for-byte unchanged."""
        if other is not None:
            return self.explain_similarity(topic, other)
        return self._explain_docs(topic)

    def _explain_docs(self, topic):
        """DOCS-DERIVED EXPLANATION, PUSHED DOWN (cp68): 'how does X work' answered
