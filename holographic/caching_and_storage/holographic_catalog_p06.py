...
def register_p06(c):
    ...
    c.register_capability("holographic_catalog", "THIS catalog: search the engine's own capabilities before building "
                          ...
                          native=False, aliases=("catalog", "capability", "registry", "find", "discover", "duplicate building register"))
    ...
    c.register_capability("Lighting (domain)", "one home for lighting: the light types (point/directional/spot/area/"
                          ...
                          native=True, aliases=("lighting", "light", "lamp", "shadow", "dome", "area", "ies", "spot",
                                                "nee", "direct", "prt", "irradiance types spot"))
    c.register_capability("Build a path-tracer light by name (aimed, one door)", "ten light classes shipped and "
                          ...
                          aliases=("add a softbox light to my scene", "area light with soft shadows",
                                   "environment lighting from a sky dome", "hdri lighting nine classes", "make a spotlight",
    ...
    c.register_capability("Shadow / visibility (domain)", "test whether light or the environment reaches a point: "
                          ...
                          native=True, aliases=("shadow", "visibility", "occlusion", "ambient occlusion visibility strategies", "penumbra",
    ...
    c.register_capability("Encoders (number to vector)", "turn raw values into hypervectors: scalar & fractional-power "
                          ...
                          native=True, aliases=("encode", "encoder", "number to vector", "scalar encoding",
                                                "fractional power encoding", "fpe", "encode coordinates", "phasor taper beta", "fhrr taper beta",
    ...
    c.register_capability("Physics & chemistry (domain)", "physical/chemical PROPERTIES and their evolution: the matter "
                          ...
                          native=True, aliases=("physics", "chemistry", "matter", "mixture", "diffusion", "material properties equilibrium film",
                                                "iridescence equilibrium oxidation", "oxidation", "phase"))
    ...
    c.register_capability("Denoise (domain)", "clean a render or signal with one home: image SVGF (variance-guided "
                          ...
                          native=True, aliases=("denoise", "svgf", "clean", "smooth", "nlm", "demodulate", "sharpen divide guided",
    ...
    c.register_capability("Memory (cache hierarchy)", "keep the hot working set where the CPU can reach it fast: FFT "
                          ...
                          native=True, aliases=("memory", "cache", "residency", "resident", "spectrum cache", "batch",
                                                "bind_batch", "backend recomputing backends", "gpu", "jit", "working set", "hot"))
    ...
    c.register_capability("Transform (warp)", "move / rotate / warp across representations: VSA bind (rigid) + "
                          ...
                          native=True, aliases=("transform", "warp", "rotate", "translate", "scale", "rigid", "affine",
                                                "matrix", "quaternion", "rotor", "bind representations quaternions", "permute representations quaternions", "gizmo",
    ...
    c.register_capability("Blend (combine)", "combine things into one: bundle (superposition, weighted = soft "
                          ...
                          native=True, aliases=("blend", "combine", "merge", "interpolate", "lerp", "slerp", "mix",
                                                "composite", "superpose", "average", "crossfade lerp things", "morph lerp things"))
    c.register_capability("Scale (distribute)", "make something bigger than one box / one pass can hold: partition a "
                          ...
                          native=True, aliases=("scale", "distribute", "partition", "map reduce sparsefield pieces", "tile", "brick",
                                                "parallel sparsefield pieces", "shard", "chunk", "monoid", "scale out"))
    ...
    c.register_capability("Token sampling (temperature + nucleus)",
                          ...
                          native=True,
                          aliases=("sample stochastic draw", "sampler", "temperature sampling", "nucleus sampling", "top p",
    ...
    c.register_capability("Optical elements (Mueller matrices)", "how optical elements TRANSFORM polarized light, as real 4x4 Mueller matrices (holographic_mueller): polarizer, wave plate / retarder (a quarter-wave plate converts linear<->circular -- the mantis R8 mechanism), optical ROTATOR (= Faraday rotation), depolarizer, and polarizing dielectric (Fresnel) reflection. Elements COMPOSE (a light path folds to one matrix) and apply to a Stokes vector or a whole field",
                          ...
                          native=True, aliases=("mueller matrix", "polarizer", "wave plate", "quarter wave plate", "half wave plate", "retarder", "optical rotator", "faraday rotation mueller retarder", "fresnel polarization", "birefringence", "transform polarized light", "polarizing filter"), semantic="transform/warp", consumes=("spectrum",), produces=("spectrum",))
    ...
    c.register_capability("Load an HDRI environment map (.hdr RGBE -> unbounded radiance)", "image-based lighting needed one missing piece and this is it: a Radiance .hdr/.pic (RGBE) reader giving UNBOUNDED linear radiance. DomeLight's color already took a callable and sky_dome already sampled an equirectangular env -- but load_image reads 8 bits, and an 8-bit env is the wrong input because an HDRI's sun is thousands of times brighter than its sky. MEASURED: a flat dome vs a procedural sky FIELD differ 0.0054 (invisible); the same env mirrored differs 0.0336. Gradients don't pay, DIRECTIONAL structure does. KEPT NEG: no .exr; XYZE raises; never clip the result",
                          ...
                          native=True, module="render",
                          aliases=("load an hdri environment map", "image based lighting piece needed",
    ...
    c.register_capability("Texture a scene object (named procedural or image, JSON-safe)", "texture a Scene object BY NAME ('wood','marble','checker',... or an (H,W,3) image, None removes) -- JSON-safe end to end, which is the point: scene_to_render already honoured an albedo_socket callable and proc_texture already built one, but a CALLABLE cannot cross POST /invoke, so over HTTP texturing was impossible while every part worked in-process. This builds the callable server-side from JSON. SOLID texture (evaluated at world points -- grain carves through, no UVs needed). KEPT NEG: albedo only; image mapping is world-XZ planar (triplanar needs normals the socket contract lacks)",
                          ...
                          aliases=("put an image on the cube", "wood grain texture on my object",
                                   ...
                                   "marble texture carves mapping", "paint a texture onto a shape",
    ...
    c.register_capability(
        ...
        native=True, aliases=("recover many items from one bundle", "find which codebook entries are in this sum",
                              ...
                              "how many things fit in a bundle correlate revisits", "pull the parts out of a sum of vectors",
    ...
    c.register_capability(
        ...
        native=True, aliases=("snap parts onto a creature", "library of body parts", "rigblock",
                              ...
                              "body part library attaching queryable", "symmetry group", "creature accessories",
    ...
    c.register_capability(
        ...
        native=True, aliases=("render a crystal", "render until converged", "one call render",
                              "denoised render", "adaptive sampling render searched svgf", "how do I render a gem",
    ...
    c.register_capability(
        ...
        native=True, aliases=("template wrap", "shrink wrap a mesh", "fixed topology",
                              "vertex correspondence", "retopology precondition morphing", "same mesh new body",
    ...
    c.register_capability(
        ...
        native=True, aliases=("tissue material", "subsurface scattering", "organ material chitin deeper",
                              "skin shader", "bone material chitin deeper", "realistic flesh", "SSS"))
    ...
    c.register_capability(
        ...
        "mind.conformal_interval(residuals, prediction, alpha=) wraps a point prediction in a "
        "distribution-free (1-alpha) band from held-out residuals (batch split-conformal; "
        "adaptive_conformal is the online cousin). mind.epistemic_map(density_a, density_b, "
        "disagreement) names the kind of not-knowing -- void / contested / confident -- so a "
        "model gathers, arbitrates, or answers; mind.vector_disagreement(a, b) feeds it. "
        "mind.semantic_compass() records what worked and steers queries toward it. Wired in "
        "sweep 123 from the import-only list: reachable by import was never reachable.",
        example="import lecore, numpy as np; m=lecore.UnifiedMind(); print(m.conformal_interval(np.abs(np.random.default_rng(0).normal(size=100)), 5.0))",
        native=True, aliases=("calibrated prediction interval", "how sure can I be about this prediction",
                             "is the evidence sparse or contested", "steer a query toward what worked",
                             "measure disagreement between two vectors"))

    c.register_capability(
        "Table history doors (a git-like timeline for a user table)",
        "mind.user_table(name, columns, dim=, seed=) is the constructor door the WRITABLE "
        "table never had; mind.table_history(table) opens a timeline (works on make_table "
        "snapshots too); table_commit(h, table, note=) snapshots; table_as_of(h, version, sql) "
        "queries the past; table_diff(h, a, b, pk_col=) names added/removed/changed rows; "
        "table_revert(h, version) restores; table_versions(h, pk_col, key) is one row's "
        "history. Sweep 123 wired holographic_querytime and caught its first bug the moment "
        "it became reachable (it assumed fields a snapshot Table never had).",
        example="import lecore; m=lecore.UnifiedMind(); u=m.user_table('t',['id','v'],dim=256); u.insert({'id':1,'v':1}); h=m.table_history(u); print(m.table_commit(h,u,'first'))",
        native=True, aliases=("what did this table look like before", "version history of a table",
                             "undo changes to a table", "create a writable table I own",
                             "diff two versions of a table"))

    c.register_capability(
        "Determinism, node client and BRDF doors (the rest of the import-only list)",
        "mind.deterministic_topk(scores, k) and mind.hash_unit(*keys) are the ISA-1 "
        "determinism contract as verbs (fixed tie rule; hashlib never hash()); "
        "mind.determinism_report(n_facts=) runs the twins probe -- two minds, identical "
        "teaching, byte-compare the saved partitions -- a False is a stop-the-release "
        "finding. mind.remote_tools(url) / mind.remote_call(url, name, args=) call another "
        "leCore node the way leCore is served. mind.brdf_terms(n.h, n.v, n.l, roughness, "
        "base_color=, metallic=) returns the GGX D, Smith G, F0 and Schlick F terms.",
        example="import lecore; m=lecore.UnifiedMind(); print(m.deterministic_topk([0.5,0.9,0.9,0.1],2), m.determinism_report(n_facts=20)['byte_identical'])",
        native=True, aliases=("check that two runs are byte identical", "top k with a fixed tie rule",
                             "call the tools on another lecore node", "deterministic number from a key",
                             "evaluate a physically based brdf", "release readiness determinism check"))

    c.register_capability(
        "Swarm use-case doors (service escalation, shared codebase sync, bus roles)",
        "Three compositions on existing rails. SERVICE: serve() records what it could not "
        "answer; escalations() lists it; resolve(q, a, by=, propagate=) teaches "
        "the human's answer with provenance human:<by>, clears the ledger, and contributes "
        "to a commons bundle so every other agent serves it next. DEVELOPMENT: "
        "codebase_sync(root, only_stale=) teaches file-fingerprinted digests that persist "
        "across restarts; stale_facts names files that changed; only_stale re-syncs just "
        "those. LAB: role(name, pattern, handler, emit=) subscribes a focused handler on "
        "the bus, sharing one memory; roles chain by topic.",
        example="import lecore; m=lecore.UnifiedMind(); m.serve('unknown thing'); print(m.escalations()[0]['question'])",
        native=True, aliases=("customer service swarm that learns from humans", "escalate to a human and remember the answer",
                             "shared understanding of the codebase across agents", "sync the swarm's knowledge of the code",
                             "a lab of agents on one bus and one memory", "focused agent role reacting to a topic"))
