...
from holographic.sampling_and_signal.holographic_fpe import (
    ...
    _BUNDLE_ENCODE_BATCH_ROWS,
    _fpe_parallel_workers,
)
...
class FractalNoise:
    ...
    def sample_grid_fast(self, res):
        """EXACTLY the same result as sample_grid(res) but far faster on a regular lattice, by exploiting that a
        grid has only `res` DISTINCT values per axis: each octave's per-axis FPE encodings are computed ONCE per
        axis-value (res encodings/axis) and then reused, instead of re-encoding every one of the res**n_dims points
        from scratch. Since binding the axes is exact (circular convolution = spectrum multiply), and the read is
        the same cosine, the output is identical to the slow path to floating point -- only the redundant repeat
        FFTs are removed (for a 40**3 grid that's ~120 encodes instead of ~192,000). Verified against sample_grid
        in the module selftest. 3-D only (the common case: cloud/volume density bakes); other n fall back to slow."""
        import numpy as _np
        from holographic.agents_and_reasoning.holographic_ai import bind, cosine
        if self.n_dims != 3:
            return self.sample_grid(res)
        axes_vals = [_np.linspace(lo, hi, res) for (lo, hi) in self.bounds]
        out = _np.zeros((res, res, res))
        for amp, enc, fld in zip(self.amplitudes, self.encoders, self.fields):
            # per-axis: encode each of the `res` distinct coordinate values ONCE -> (res, dim) tables
            ex = _np.stack([enc.axes[0].encode(v) for v in axes_vals[0]])   # (res, dim)
            ey = _np.stack([enc.axes[1].encode(v) for v in axes_vals[1]])
            ez = _np.stack([enc.axes[2].encode(v) for v in axes_vals[2]])
            fld = _np.asarray(fld)
            # bind is circular convolution: fft-multiply the three per-axis spectra, one batched read per (i,j).
            Fx = _np.fft.rfft(ex, axis=1); Fy = _np.fft.rfft(ey, axis=1); Fz = _np.fft.rfft(ez, axis=1)
            Ffld = _np.fft.rfft(fld); fld_norm = _np.linalg.norm(fld)
            for i in range(res):
                # bind x_i with every y: (res, dimf) spectra
                Fxy = Fx[i][None, :] * Fy                                    # (res, dimf)
                for j in range(res):
                    # bind (x_i,y_j) with every z, then cosine with the field == real-ifft dot, done in spectrum:
                    # dot(bind(e), fld) = sum real(ifft(Fe*Ffld)); but cosine needs |bind(e)| per z (each unit-ish).
                    Fxyz = Fxy[j][None, :] * Fz                             # (res, dimf) spectra of bind over z
                    enc_vecs = _np.fft.irfft(Fxyz, n=enc.dim, axis=1)        # (res, dim) spatial
                    norms = _np.linalg.norm(enc_vecs, axis=1)
                    dots = enc_vecs @ fld
                    # cosine(fld, encode) = dot / (|fld| |encode|); matches query()'s cosine read exactly
                    denom = _np.where(norms > 0, norms, 1.0) * (fld_norm if fld_norm > 0 else 1.0)
                    out[i, j, :] += amp * (dots / denom)
...
def _selftest():
    ...
    # (3b) FAST BAKE EXACTNESS: sample_grid_fast must equal sample_grid to floating point (it only removes the
    #      redundant repeat encodings on a regular lattice). Guards the ~40x cloud-bake speedup.
    fbm3 = FractalNoise(3, dim=512, bounds=[(-1.5, 1.5)] * 3, octaves=3, lacunarity=2.0, gain=0.5, seed=7)
    g_slow = fbm3.sample_grid(7)
    g_fast = fbm3.sample_grid_fast(7)
    assert np.allclose(g_slow, g_fast, atol=1e-10), "sample_grid_fast must match sample_grid exactly"
